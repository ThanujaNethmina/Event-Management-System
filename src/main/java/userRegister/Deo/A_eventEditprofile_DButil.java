...
public class A_eventEditprofile_DButil {
	public static List<A_eventUploader> Validate(String User_N,String Pass_w){
		  ...
		  try {
			  ...
			  String sql = "SELECT * FROM onlineeventmngsystwm.eventuploader WHERE username = '"+User_N+"' AND password = '"+Pass_w+"'";
			  ResultSet res = Stat.executeQuery(sql);
			  
			  if(res.next()) {
				  int u_id = res.getInt(1);
				  String u_firstName = res.getString(2);
				  ...
				  A_eventUploader U1 = new A_eventUploader(u_id, u_firstName, u_lastName, u_username, u_password, u_Eaddress, u_contact);
				  user.add(U1);
			  }
		  }
		  catch(Exception e) {
